...
class OpencvFuncs:
    ...
    def __init__(self, project_path, base_ctrl):
        ...
        self.faceCascade = cv2.CascadeClassifier(
            thisPath + "/models/haarcascade_frontalface_default.xml"
        )
        ...
        self.net = cv2.dnn.readNetFromCaffe(
            thisPath + "/models/deploy.prototxt",
            thisPath + "/models/mobilenet_iter_73000.caffemodel",
        )
        self.class_names = [
            "background",
            "aeroplane",
        ...
        if not self.usb_camera_connected and not self.csi_camera_connected:
            try:
                ...
                self.output_queue = self.device.getOutputQueue(
                    name="video", maxSize=8, blocking=False
                )
    ...
    def frame_process(self):
        try:
            ...
            ret, buffer = cv2.imencode(
                ".jpg", input_frame, [int(cv2.IMWRITE_JPEG_QUALITY), self.video_quality]
            )
            input_frame = buffer.tobytes()
            return input_frame
        ...
        if self.cv_mode != f["code"]["cv_none"]:
            ...
            cv2.rectangle(
                self.overlay,
                (round((self.info_scale - 0.005) * 640), round((0.33) * 480)),
            ...
            for i in range(0, len(self.info_deque)):
                cv2.putText(
                    input_frame,
                    str(self.info_deque[i]["text"]),
                    (
                        round(self.info_scale * 640),
                        round(self.info_scale * 640 - i * 20),
                    ),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    self.info_deque[i]["size"],
                    self.info_deque[i]["color"],
                    1,
                )
        ...
        if not self.set_video_record_flag and not self.video_record_status_flag:
            ...
            self.writer.append_data(
                np.array(cv2.cvtColor(input_frame, cv2.COLOR_BGRA2RGB))
            )
    ...
    def osd_render(self, osd_frame):
        ...
        # render lidar data
        lidar_points = []
        for lidar_angle, lidar_distance in zip(
            self.base_ctrl.rl.lidar_angles_show, self.base_ctrl.rl.lidar_distances_show
        ):
    ...
    def cv_detect_faces(self, img):
        ...
        faces = self.faceCascade.detectMultiScale(
            gray_img, scaleFactor=1.2, minNeighbors=5, minSize=(20, 20)
        )
        overlay_buffer = np.zeros_like(img)
        ...
        if len(faces):
            if self.cv_light_mode == 1:
                if self.base_ctrl.head_light_status == 0:
                    ...
                    self.base_ctrl.lights_ctrl(
                        self.base_ctrl.base_light_status,
                        self.base_ctrl.head_light_status,
                    )
            ...
            if self.detection_reaction_mode == f["code"]["re_reco"]:
                if (
                    datetime.datetime.now() - self.last_frame_capture_time
                ).seconds >= 5:
                    self.video_record(False)

        cv2.putText(
            overlay_buffer,
            "NUMBER: {}".format(len(faces)),
        ...
        cv2.putText(
            overlay_buffer,
            "ITERATE: {}".format(self.track_faces_iterate),
        ...
        cv2.putText(
            overlay_buffer,
            " SPD_R: {}".format(self.track_spd_rate),
        ...
        cv2.putText(
            overlay_buffer,
            " ACC_R: {}".format(self.track_acc_rate),
    ...
    def cv_detect_objects(self, img):
        ...
        for i in range(0, detections.shape[2]):
            ...
            if confidence > 0.2:
                ...
                cv2.rectangle(
                    overlay_buffer, (startX, startY), (endX, endY), (0, 255, 0), 2
                )
    ...
    def cv_detect_color(self, img):
        ...
        cv2.putText(
            overlay_buffer,
            "ITERATE: {}".format(self.track_color_iterate),
        ...
        cv2.circle(
            overlay_buffer, (center_x, center_y), self.sampling_rad, (64, 255, 64), 1
        )
        ...
        if len(cnts) > 0:
            ...
            if radius > self.min_radius:
                if not self.cv_movtion_lock:
                    distance = self.gimbal_track(
                        center_x,
                        center_y,
                        center[0],
                        center[1],
                        self.track_color_iterate,
                    )
                    if distance < self.aimed_error:
                        head_light_pwm = 10
                        self.base_ctrl.lights_ctrl(
                            self.base_ctrl.base_light_status, head_light_pwm
                        )
                    else:
                        head_light_pwm = 0
                        self.base_ctrl.lights_ctrl(
                ...
                cv2.circle(
                    overlay_buffer, (int(x), int(y)), int(radius), (128, 255, 255), 1
                )
        ...
        if results.multi_hand_landmarks:
            for handLms in results.multi_hand_landmarks:
                ...
                self.mpDraw.draw_landmarks(
                    overlay_buffer, handLms, self.mpHands.HAND_CONNECTIONS
                )

                target_pos = handLms.landmark[
                ...
                if middle_finger_gs > 20 and pinky_finger_gs > 90:
                    ...
                    tips_distance = self.calculate_distance(
                        handLms.landmark[self.mpHands.HandLandmark.INDEX_FINGER_TIP],
                    ...
                    get_pwm = int(
                        self.map_value(tips_distance, 0.01, self.max_distance, 0, 128)
                    )
                ...
                # Take Pic
                    ...
                    middle_finger_gs < 10
                    and pinky_finger_gs > 90
                    and index_finger_gs < 10
                ):
                    cv2.putText(
                        overlay_buffer,
                        " GS: Take Pic",
    ...
    def cv_auto_drive(self, img):
        ...
        input_turning = 0
        if sam_1 and sam_2:
            line_slope = (sampling_1_center - sampling_2_center) / abs(
                sampling_h1 - sampling_h2
            )
            ...
            input_turning = -(
                line_slope * self.slope_impact
                + (sampling_2_center - center_x) * self.base_impact
            )  # + (speed_impact * input_speed)
        ...
        if not self.cv_movtion_lock:
            self.base_ctrl.base_json_ctrl(
                {"T": 13, "X": input_speed, "Z": input_turning}
            )
        ...
        cv2.putText(
            overlay_buffer,
            " SAM_H1: {}".format(self.sampling_line_1),
            (center_x - 150, sampling_h1 - 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 128, 128),
            1,
        ...
        cv2.putText(
            overlay_buffer,
            " SAM_H2: {}".format(self.sampling_line_2),
            (center_x - 150, sampling_h2 - 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 128, 128),
            1,
        ...
        cv2.putText(
            overlay_buffer,
            f"X: {input_speed:.2f}, Z: {input_turning:.2f}",
        ...
        cv2.putText(
            overlay_buffer,
            f" SAM_1 SAM_2 SLOPE_IM BASE_IM SPD_IM LT_SPD SLOPE_SPD",
            (center_x - 250, center_y - 70),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 128, 128),
            1,
        ...
        cv2.putText(
            overlay_buffer,
            f" {self.sampling_line_1:.2f}   {self.sampling_line_2:.2f}   {self.slope_impact:.2f}      {self.base_impact:.4f}  {self.speed_impact:.2f}    {self.line_track_speed:.2f}    {self.slope_on_speed:.2f}",
        ...
        if sam_1:
            cv2.line(
                overlay_buffer,
                (sampling_1_left, sampling_h1 + 20),
                (sampling_1_left, sampling_h1 - 20),
                (0, 255, 0),
                2,
            )
            cv2.line(
                overlay_buffer,
                (sampling_1_right, sampling_h1 + 20),
                (sampling_1_right, sampling_h1 - 20),
                (0, 255, 0),
                2,
            )
        if sam_2:
            cv2.line(
                overlay_buffer,
                (sampling_2_left, sampling_h2 + 20),
                (sampling_2_left, sampling_h2 - 20),
                (0, 255, 0),
                2,
            )
            cv2.line(
                overlay_buffer,
                (sampling_2_right, sampling_h2 + 20),
                (sampling_2_right, sampling_h2 - 20),
                (0, 255, 0),
                2,
            )
        if sam_1 and sam_2:
            cv2.line(
                overlay_buffer,
                (sampling_1_center, sampling_h1),
                (sampling_2_center, sampling_h2),
                (255, 0, 0),
                2,
            )
    ...
    def mediaPipe_pose(self, img):
        ...
        if results.pose_landmarks:
            self.mpDraw.draw_landmarks(
                overlay_buffer, results.pose_landmarks, self.mp_pose.POSE_CONNECTIONS
            )
        self.overlay = overlay_buffer
    ...
    def update_base_data(self, input_data):
        ...
        try:
            if self.show_base_info_flag:
                self.recv_deque.appendleft(
                    json.dumps(self.format_json_numbers(input_data))
                )
            if input_data["T"] == 1003:
                self.info_deque.appendleft(
                    {
                        "text": json.dumps(input_data["mac"]),
                        "color": (16, 64, 255),
                        "size": 0.5,
                    }
                )
                wrapped_lines = textwrap.wrap(
                    json.dumps(input_data["megs"]), self.recv_line_max
                )
                for line in wrapped_lines:
                    self.info_deque.appendleft(
                        {"text": line, "color": (255, 255, 255), "size": 0.5}
                    )
                self.info_update_time = time.time()
                self.show_info_flag = True
        except Exception as e:
    ...
    def opencv_threading(self, input_img):
        cv_thread = threading.Thread(
            target=self.cv_process, args=(input_img,), daemon=True
        )
        cv_thread.start()
    ...
    def set_line_track_args(
        self, sam_pos_1, sam_pos_2, slope_im, base_im, spd_im, lt_spd, slope_spd
    ):
